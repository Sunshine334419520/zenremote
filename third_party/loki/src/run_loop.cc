...
RunLoop::Delegate::~Delegate() {
  if (bound_) {
    //tls_delegate.private_instance_.store(nullptr, std::memory_order_relaxed);
    *tls_delegate.Pointer() = nullptr;
  }
}
...
void RunLoop::RegisterDelegateForCurrentThread(Delegate* delegate) {
  ...
  //tls_delegate.private_instance_.store(delegate);
  //tls_delegate.Get()->active_run_loops_.size();
  *tls_delegate.Pointer() = delegate;
...
void RunLoop::Quit() {
  ...
  if (!origin_task_runner_->RunsTasksInCurrentSequence()) {
    origin_task_runner_->PostTask(FROM_HERE,
                                  loki::BindOnceClosure(&RunLoop::Quit, this));
    return;
  }
...
void RunLoop::QuitWhenIdle() {
  ...
  if (!origin_task_runner_->RunsTasksInCurrentSequence()) {
    origin_task_runner_->PostTask(
        FROM_HERE, loki::BindOnceClosure(&RunLoop::QuitWhenIdle, this));
    return;
  }
...
Closure RunLoop::QuitClosure() {
  ...
  /*return loki::BindClosure(&ProxyToTaskRunner, origin_task_runner_,
                                   loki::BindOnceClosure(&RunLoop::Quit,
     this));*/
  return Closure();
}
...
Closure RunLoop::QuitWhenIdleClosure() {
  ...
  /*return loki::BindClosure(&ProxyToTaskRunner, origin_task_runner_,
                                   loki::BindOnceClosure(&RunLoop::QuitWhenIdle,
     this));*/
  return Closure();
}
...
OnceClosure RunLoop::QuitCurrentWhenIdleClosureDeprecated() {
  ...
  //        "QuitWhenIdleClosure().";
  return loki::BindOnceClosure(&RunLoop::QuitCurrentWhenIdleDeprecated);
}
