...
class LOKI_EXPORT RunLoop {
 public:
  // 顶层的kDefault RunLoop non-nested会处理绑定到它自己的系统和应用级别的任务,
  // 当处理一个嵌套时，kDefault
  // RunLoop将只会处理系统任务，而kNestableTasksAllowed
  // RunLoop将会处理应用级别的任务.
  //
  // 在默认的情况下，嵌套任务将会被禁止.
  ...
  // 不会有影响的.
  //
  // WARNING: 你必须确保调用Quit() or QuitWhenIdle() 来终止RunLoop和message
  // loop, 如果你不调用，我们又启动了一个nestable RunLoop ，那样就会导致message
  // loop永远都 不会退出.
  void Quit();
  void QuitWhenIdle();
  ...
  class LOKI_EXPORT NestingObserver {
   public:
    // 通知再当前线程开始运行任务工作之前.
