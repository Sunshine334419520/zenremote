...
namespace {
thread_local loki::LazyInstance<bool>::Leaky lazy_tls_bool;
}
...
bool Thread::WaitUntilThreadStarted() const {
  ...
  start_event_.wait(lock, [this]() { return running_; });
  return true;
}
...
void Thread::StopSoon() {
  ...
  task_runner()->PostTask(FROM_HERE,
                          BindOnceClosure(&Thread::ThreadQuitHelper, this));
}

PlatformThreadId Thread::GetThreadId() const {
  std::unique_lock<std::mutex> lock(cond_var_mutex_);
  id_event_.wait(lock, [this]() { return running_; });
  return id_;
}
...
PlatformThreadHandle Thread::GetThreadHandle() {
  ...
  return is_thread_valid_ ? thread_.native_handle() : static_cast<std::thread::native_handle_type>(NULL);
}
